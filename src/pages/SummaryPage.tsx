...
import {
  ...
  getNetPayments, // ایمپورت تابع جدید
...
export default function SummaryPage({
  ...
  // فراخوانی تابع جدید برای دریافت پرداخت خالص
  const netPayments = getNetPayments(expenses, travelers, manualPayments);

  const handleAddManualPayment = (
    paymentData: Omit<ManualPayment, "id" | "createdAt">
  ) => {
  ...
  if (expenses.length === 0 && manualPayments.length === 0) {
    return (
      <div className="text-center py-12">
        ...
        <h3 className="text-xl font-semibold text-gray-600 mb-2">
          هنوز تراکنشی برای نمایش خلاصه وجود ندارد
        </h3>
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Net Paid Per Person Card */}
      <SummaryCard
        title="پرداخت خالص هر نفر"
        className="bg-gradient-to-r from-green-50 to-green-100"
      >
        <div className="space-y-4">
          {travelers.map((traveler) => {
            const paidAmounts = netPayments[traveler.id] || {};
            const hasTransactions = Object.values(paidAmounts).some(
              ...
              >
                ...
                <div className="text-left">
                  {hasTransactions ? (
                    ...
                    <span className="text-gray-500 text-sm">بدون تراکنش</span>
                  )}
                </div>
              </div>
            );
      ...
      >
        {settlements.length === 0 ? (
          <div className="text-center py-8">
            ...
            <p className="text-gray-600">هیچ بدهی باقی نمانده است</p>
          </div>
        ) : (
          <div className="space-y-3">
            {settlements.map((settlement, index) => {
              ...
              const currency = CURRENCIES.find(
                (c) => c.value === settlement.currency
              );

              return (
                <div
                  key={index}
                  className="flex items-center justify-between p-4 bg-white rounded-lg shadow-sm border-r-4 border-orange-400"
                >
                  <div className="flex flex-wrap items-center gap-x-3 gap-y-1">
                    <div className="flex items-center gap-2">
                      <div className="w-10 h-10 rounded-full overflow-hidden bg-gradient-to-r from-orange-400 to-orange-600 flex items-center justify-center text-white flex-shrink-0">
      ...
      <div className="space-y-4">
        <ManualPaymentForm
          travelers={travelers}
          onAdd={handleAddManualPayment}
        />

        {manualPayments.length > 0 && (
          <SummaryCard
            title="پرداخت‌های دستی ثبت شده"
            className="bg-gradient-to-r from-purple-50 to-purple-100"
          >
            <div className="space-y-3">
              {manualPayments.map((payment) => {
                const payer = travelers.find((t) => t.id === payment.payerId);
                const receiver = travelers.find(
                  (t) => t.id === payment.receiverId
                );
                const currency = CURRENCIES.find(
                  (c) => c.value === payment.currency
                );
